...
public class SupervisorControllerTest {
    ...
    @Test
    @DisplayName("If we create topic with parameters we get status 300 redirect")
    void test_3() throws Exception {
        mvc.perform(post("/supervisor/createTopic")
                        .param("topic", "Bruce Wayne")
    ...
    @Test
    @DisplayName("Title of topic correctly shown on start page")
    ...
    @Test
    @DisplayName("Description of topic correctly shown on start page")
    ...
    @Test
    @DisplayName("When clicking edit on any topic the status is 200")
    void test_8() throws Exception {
        Topic topic = createBruce();
        when(service.editTopic(any())).thenReturn(topic);

        mvc.perform(get("/supervisor/editTopic/2"))
                .andExpect(successfullWithAttribute("topic"));

    }

    @Test
    @DisplayName("When clicking edit on any topic the topicedit view is shown")
    ...
    @Test
    @DisplayName("When clicking edit on any topic an attribute called 'topic' exists")
    ...
    void test_11() throws Exception {
        when(service.editTopic(any())).thenReturn(createBruce());
        mvc.perform(get("/supervisor/editTopic/1"));
        verify(service)
                .editTopic(argThat(command -> command.supervisorId().equals(1L) && command.topicId().equals(1L)));
    }
    ...
    @Test
    @DisplayName("If get request at /supervisor/topicform is made a form exists with inputs 'id', 'topic', 'description'")
    void test_17() throws Exception {
        MvcResult result = mvc.perform(get("/supervisor/topicform")).andReturn();
        //Jsoup
        Document html = Jsoup.parse(result.getResponse().getContentAsString());
        assertThat(html.select("input[name=topic],[name=description],[name=id]")).hasSize(3);
    }
}

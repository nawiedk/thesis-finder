...
import com.devsxplore.thesis.supervisors.application.port.in.command.topic.EditTopicCommand;
...
public class SupervisorServiceTest {
    ...
    @Test
    @DisplayName("If createTopic is invoked Topic object with correct attributes is returned/Supervisor title list contains correct object")
    void test_9() {
        Supervisor supervisor = createSupervisorWithNoTopics();
        when(repositoryPort.load(any())).thenReturn(supervisor);
        when(repositoryPort.save(any())).thenReturn(supervisor);

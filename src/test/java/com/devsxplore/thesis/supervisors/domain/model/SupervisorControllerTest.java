...
import com.devsxplore.thesis.supervisors.adapter.in.web.SupervisorController;
import com.devsxplore.thesis.supervisors.adapter.in.web.dto.TopicForm;
...
import static com.devsxplore.thesis.supervisors.domain.model.TopicFactory.*;
...
public class SupervisorControllerTest {
    ...
    void test_4() throws Exception {
        when(service.loadTopicsBySupervisor(any())).thenReturn(createBruceAndAlfred());
    ...
    void test_14() throws Exception {
        ...
        verify(service)
                .updateTopic(argThat(command -> command.topicId().equals(1L)
                        && command.title().equals("Bruce Wayne")
                        && command.description().equals("Batman")));
    }
    ...
    @Test
    @DisplayName("If get request at /supervisor/topicform is made a form exists with inputs 'supervisorId', 'title', 'description'")
    void test_17() throws Exception {
        MvcResult result = mvc.perform(get("/supervisor/topicform")).andReturn();
        //Jsoup
        Document html = Jsoup.parse(result.getResponse().getContentAsString());
        assertThat(html.select("input[fieldName=title],[fieldName=description],[fieldName=supervisorId]")).hasSize(3);
    }
}

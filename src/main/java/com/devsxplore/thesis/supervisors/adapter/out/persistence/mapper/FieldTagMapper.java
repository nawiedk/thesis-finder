...
import com.devsxplore.thesis.supervisors.adapter.out.persistence.jdbcentity.FieldTagJDBCEntity;
...
public class FieldTagMapper {
	...
	public static Set<FieldTag> mapFieldTagsToDomainEntities(Set<FieldTagJDBCEntity> entities) {
		return entities.stream().map(FieldTagMapper::mapFieldTagToDomainEntity).collect(Collectors.toSet());
	}

	public static Set<FieldTagJDBCEntity> mapFieldTagsToJDBCEntities(Set<FieldTag> entities) {
		return entities.stream().map(FieldTagMapper::mapFieldTagToJDBCEntity).collect(Collectors.toSet());
	}
